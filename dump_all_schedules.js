...
async function run() {
    const asc = await supabase.from('area_schedules').select('*');
    const col = await supabase.from('collection_schedules').select('*');
    
    fs.writeFileSync('schedules_dump.json', JSON.stringify({
        area_schedules: asc.data,
        collection_schedules: col.data
    }, null, 2));
