-- ===============================================
-- FINAL UNIFIED SCHEMA FIX (v3 - REMOVED DATES)
-- ===============================================
-- This script fixes "resident_id" and removes "preferred_date"
...
BEGIN
    ...
    IF EXISTS (SELECT FROM pg_tables WHERE tablename = 'special_collections') THEN
        ...
        -- Remove preferred_date and preferred_time if they exist
        ...
        IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'special_collections' AND column_name = 'preferred_time') THEN
            ALTER TABLE special_collections DROP COLUMN preferred_time;
        END IF;

        -- Add other missing columns needed by the app
...
-- 2. FIX USER_NOTIFICATIONS TABLE
